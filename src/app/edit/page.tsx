            ...
            {event?.custom_consent_enabled && (
              <div className="flex items-start gap-2.5 p-3.5 bg-slate-50 rounded-2xl border border-slate-100">
                <input
                  ...
                  className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 mt-0.5 disabled:opacity-60"
                />
                <label htmlFor="customConsentAgreed" className="text-xs text-slate-600 leading-relaxed cursor-pointer select-none">
                  <span className="font-bold text-slate-800">
                    [{event.custom_consent_required ? '필수' : '선택'}] {event.custom_consent_title || '추가 동의서'}
                  </span>
                  {event.custom_consent_content && (
                    <p className="text-[10px] text-slate-400 mt-1 whitespace-pre-wrap leading-relaxed font-medium">{event.custom_consent_content}</p>
                  )}
                </label>
              </div>
            )}
