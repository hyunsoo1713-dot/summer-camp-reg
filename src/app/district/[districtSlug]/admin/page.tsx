                        ...
                        <tr key={m.id} className="hover:bg-slate-50/50">
                          ...
                          <td className="p-3 text-center">
                            <div className="flex justify-center items-center gap-2 flex-wrap">
                              ...
                              <button
                                type="button"
                                onClick={() => handleToggleAdmin(m.id, m.is_admin ?? (m.church_id === ''))}
                                className={`font-bold py-1 px-2.5 rounded text-[10px] border transition-all-custom ${
                                  (m.is_admin ?? (m.church_id === ''))
                                    ? 'bg-indigo-600 border-indigo-600 text-white hover:bg-indigo-700'
                                    : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                                }`}
                              >
                                지방 관리자 {(m.is_admin ?? (m.church_id === '')) ? 'ON' : 'OFF'}
                              </button>

                              {/* 교회 담당자 토글 */}
                              <button
                                type="button"
                                onClick={() => handleToggleManager(m.id, m.is_manager ?? (m.church_id !== ''))}
                                className={`font-bold py-1 px-2.5 rounded text-[10px] border transition-all-custom ${
                                  (m.is_manager ?? (m.church_id !== ''))
                              ...
                              >
                                교회 담당자 {(m.is_manager ?? (m.church_id !== '')) ? 'ON' : 'OFF'}
                              </button>
